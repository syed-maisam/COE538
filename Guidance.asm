...
SENS_A          dc.B  0                         ; SENSOR A detection (PATH = 1, NO PATH = 0)
SENS_B          dc.B  0                         ; SENSOR B detection (PATH = 1, NO PATH = 0)
SENS_C          dc.B  0                         ; SENSOR C detection (PATH = 1, NO PATH = 0)
SENS_D          dc.B  0                         ; SENSOR D detection (PATH = 1, NO PATH = 0)
SENS_E          dc.B  0                         ; SENSOR E detection (PATH = 1, NO PATH = 0)
SENS_F          dc.B  0                         ; SENSOR F detection (PATH = 1, NO PATH = 0)
...
_Startup:                                       
                ...
                JSR   initPORTS                                
                JSR   initAD                       ; initialize ATD convertor                                                                              A
                JSR   initLCD                      ; initialize ldc
                JSR   clrLCD                    
                                                
                JSR   initTCNT                     ; initialize tcnt
                                                                                           
                CLI                            
                LDX   #msg1                        ; display msg 1
                JSR   putsLCD                   
...
;* state section                                                                         
;********************************************************************************************
DISPATCHER      CMPA  #START                    ; if start
                BNE   NOT_START                                                            
                JSR   START_ST                  
                RTS                                                              

NOT_START       CMPA  #FWD                      ; if forward
                BNE   NOT_FORWARD                                                         
                JMP   FWD_ST                     

NOT_FORWARD     CMPA  #RT_TRN                   ; if right turn
                BNE   NOT_RT_TRN                                                           
                JSR   RT_TRN_ST                 
                RTS                             
                                          
NOT_RT_TRN      CMPA  #LT_TRN                   ; if left turn
                BNE   NOT_LT_TRN                
                JSR   LT_TRN_ST                 
                RTS                            

NOT_LT_TRN      CMPA  #REV                      ; if reverse 
                BNE   NOT_REVERSE               
                JSR   REV_ST                   
                RTS                             

NOT_REVERSE     CMPA  #BK_TRK                   ; if backtrack
                BNE   NOT_BK_TRK                
                JMP   BK_TRK_ST                 

NOT_BK_TRK      CMPA  #SBY                      ; if standby
                BNE   NOT_SBY                   
                JSR   SBY_ST                    
                RTS                             

NOT_SBY         NOP                             ; else
DISP_EXIT       RTS                             ; return
...
NO_RT_INTXN     LDAA  SENS_B                    ; if B_DETN equals 1
                BEQ   NO_LT_INTXN               
                LDAA  SENS_A                    ; If A_DETN equals 1 
                BEQ   LT_TURN                   ; go forward
                LDAA  NEXT_D                    
                PSHA                            ; push stack
                LDAA  PRI_PTH_INT                
                STAA  NEXT_D                     
                BRA   NO_SHFT_LT                ; if A_DETN equals 0
LT_TURN         LDAA  NEXT_D                    
                PSHA                            ; push stack
                LDAA  SEC_PTH_INT               
                STAA  NEXT_D                     
                JSR   INIT_LT_TRN               ; left turn
                MOVB  #LT_TRN,CRNT_STATE        ; Initialize the LT_TRN state
                JMP   FWD_EXIT                  ; Set CRNT_STATE to LT_TRN and exit

NO_LT_INTXN     LDAA  SENS_C                    ; if F_DETN equals 1
                BEQ   NO_SHFT_RT                ; shift RIGHT
                JSR   PORTON                    ; LEFT motor
...
NO_SHFT_RT      LDAA  SENS_C                    ; if E_DETN equals 1
                BEQ   NO_SHFT_LT                ; shift LEFT
                JSR   STARON                    ; RIGHT motor
...
;--------------------------------------------------------------------------------------------
REG_PATHING     LDAA  SENS_C                    ; If D_DETN equals 1
...
NO_RT_TRN       LDAA  SENS_B                    ; If B_DETN equals 1
                BEQ   RT_LINE_S                 ; Check if A_DETN equals 1
                LDAA  SENS_A                    ; If A_DETN equals 1 a FORWARD path exists
...
LEFT_TURN       PULA                            ; The robot should make a LEFT turn
                ...
                JMP   BK_TRK_EXIT               ; and exit
;--------------------------------------------------------------------------------------------
IRREG_PATHING   LDAA  SENS_B                    ; If B_DETN equals 1
...
NO_LT_TRN       LDAA  SENS_C                    ; If D_DETN equals 1
                BEQ   RT_LINE_S                 ; Check if A_DETN equals 1
                LDAA  SENS_A                    ; If A_DETN equals 1 a FORWARD path exists
...
RIGHT_TURN      PULA                            ; The robot should make a RIGHT turn
                ...
                JMP   BK_TRK_EXIT               ; Then exit
;--------------------------------------------------------------------------------------------
RT_LINE_S       LDAA  SENS_C                    ; Else if F_DETN equals 1
...
LT_LINE_S       LDAA  SENS_C                    ; Else if F_DETN equals 1
...
CON_EXIT    RTS ;We’re done the conversion                             ; We’re done the conversion
;********************************************************************************************
